...
public class LoginService {
    ...
    private static final long EXPIRE_TIME_IN_MILLIS = 2L * 40000L;
    ...
    public VoterOutput checkTokenByVoterId(Long voterId){
        if(StringUtils.isBlank(voterId.toString())){
            throw new GenericOutputException(INVALID_TOKEN);
        }

        Token token = tokenRepository.findFirstByVoter_idOrderByExpireDateDesc(voterId);
        if (token == null){
            throw new GenericOutputException(INVALID_TOKEN);
        }
